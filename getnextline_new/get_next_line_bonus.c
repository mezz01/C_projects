...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/24 20:35:04 by mmezzat           #+#    #+#             */
/*   Updated: 2023/02/24 20:51:38 by mmezzat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buffer[fd])
		return (NULL);
	return_line = read_one_line_first(buffer[fd]);
	if (!return_line)
		return (free_str_mem(&buffer[fd]));
	buffer[fd] = extract_previous_line(buffer[fd]);
	return (return_line);
}
